...
if [ "$ID" == ubuntu ] && [ x"${TEST_INITRD}" == x"yes" ] && [ "$VERSION_ID" != "16.04" ] && [ "$MACHINETYPE" != "pseries" ]; then
	echo "Set virtio-blk as the block device driver on $ID on $VERSION_ID on $MACHINETYPE"
